use godot::{
    classes::{GeometryInstance3D, MeshInstance3D},
    prelude::*,
};

use super::{
    GodotError, JcResourceError, JcResourceFormat, JcResourceResult, JcResourceThread, model,
};

pub fn register() -> JcResourceFormat {
    (GString::from("lod"), load)
}
    ...
    if lines.len() >= 6 {
        ...
        for lod_level in 1..6 {
            ...
            if next_lod && !previous_lod.ends_with('-') {
                let lod_path = GString::from(previous_lod);
                let buffer = thread.get_buffer(&lod_path)?;
                let mut mesh = model::load(lod_path, buffer, thread)?.cast::<MeshInstance3D>();
